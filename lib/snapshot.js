...
module.exports = function snapshot(coreV, router) {
    ...
    router.post('/img/:src(' + core.Sources.sourceIdReStr + '),:zoom(\\d+),:lat([-\\d\\.]+),:lon([-\\d\\.]+),:w(\\d+)x:h(\\d+).:format([\\w]+)', requestHandler);

    router.get('/img/:src(' + core.Sources.sourceIdReStr + '),:zoom(\\d+),:lat([-\\d\\.]+),:lon([-\\d\\.]+),:w(\\d+)x:h(\\d+)@:scale([\\.\\d]+)x.:format([\\w]+)', requestHandler);
    router.post('/img/:src(' + core.Sources.sourceIdReStr + '),:zoom(\\d+),:lat([-\\d\\.]+),:lon([-\\d\\.]+),:w(\\d+)x:h(\\d+)@:scale([\\.\\d]+)x.:format([\\w]+)', requestHandler);

    router.get('/img/:src(' + core.Sources.sourceIdReStr + '),:zoom(\\d+),:lat([-\\d\\.]+),:lon([-\\d\\.]+),:w(\\d+)x:h(\\d+),:vt([\\w]+),:moving(\\d),:cog([-\\d\\.]+),:ms([\\d\\.]+).:format([\\w]+)', requestHandler);
    router.get('/img/:src(' + core.Sources.sourceIdReStr + '),:zoom(\\d+),:lat([-\\d\\.]+),:lon([-\\d\\.]+),:w(\\d+)x:h(\\d+),:vt([\\w]+),:moving(\\d),:cog([-\\d\\.]+),:ms([\\d\\.]+)@:scale([\\.\\d]+)x.:format([\\w]+)', requestHandler);
};
...
function requestHandler(req, res, next) {
    ...
    params.format = allowed_formats[params.format];
    ...
    if (!req.body.type === 'FeatureCollection' && !params.vt) {
    ...
    let geojson = req.body.type === 'FeatureCollection' ? req.body : {
        ...
        features: [
            {
                ...
                properties: {
                    'marker-url': 'https://static.fleetmon.com/static/images/vmap_icon/' + (vessel_types[params.vt] || 'unknown') + '_' + (params.moving === '1' ? 'm' : 's') + 'h.png',
                    'marker-rotate': core.strToFloat(params.cog),
                    'marker-scale': core.strToFloat(params.ms)
                }
            }
        ]
    };
